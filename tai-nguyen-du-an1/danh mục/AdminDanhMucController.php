    ...
    public function postAddDanhMuc()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (empty($errors)) {
                ...
                $this->modelDanhMuc->insertDanhMuc(ten_danh_muc: $ten_danh_muc, mo_ta: $mo_ta, trang_thai: $trang_thai );
            ...
            if (empty($trang_thai)) {
                $errors['trang_thai'] = 'Tên danh mục không được để trống';
            }
        }
    }
    ...
    public function postEditAddDanhMuc()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (empty($errors)) {
                // Nếu không có lỗi thì tiến hành thêm danh mục
                // var_dump('ok');
                $this->modelDanhMuc->updateDanhMuc($id, $ten_danh_muc, $mo_ta, trang_thai: $trang_thai);
                header('location: ' . BASE_URL_ADMIN . '?act=danh-muc');
                exit();
            } else {
                // Trả về form và lỗi
